      ...
      {decision.decisions && decision.decisions.length > 0 && (
        <div className="space-y-2 mb-3">
          {decision.decisions.map((action, j) => (
            <div key={j}>
              <div className="flex items-center gap-2 text-sm rounded px-3 py-2" style={{ background: '#0B0E11' }}>
              ...
              {(action as any).reasoning && (
                <div className="mt-2 rounded p-3 text-xs font-mono" style={{ background: '#0B0E11', border: '1px solid #2B3139', color: '#EAECEF' }}>
                  <div className="text-xs mb-2" style={{ color: '#F0B90B' }}>🤖 AI推理过程:</div>
                  <div className="whitespace-pre-wrap" style={{ whiteSpace: 'pre-wrap' }}>
                    {((action as any).reasoning || '').replace(/\\n/g, '\n')}
                  </div>
                </div>
              )}
            </div>
