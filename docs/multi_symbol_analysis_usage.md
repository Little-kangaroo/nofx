...
**交易员输出格式已更新为结构化JSON格式！** 现在交易员会自动输出你想要的数据结构格式，而不是之前的文本格式。

**✨ 新增功能：所有时间框架现在都包含完整的基础指标！**
- 15分钟/30分钟/1小时时间框架现在也包含 EMA20、EMA50、MACD、RSI7、RSI14、ATR14 等基础指标
- 基础指标不再局限于3分钟和4小时级别
...
```json
{
  "BTCUSDT": {
    "基础指标": {
      ...
      "current_ema20": 97890.23,
      "current_macd": 45.67,
      "current_rsi7": 65.43,
      "price_change_1h": 1.23,
      "price_change_4h": -2.45,
      "open_interest": {...},
      "funding_rate": 0.0001,
      "intraday_series": {...},
      "longer_term_context": {...},
