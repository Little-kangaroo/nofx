...
func TestProductionRecords(t *testing.T) {
	...
	stats := &TestStats{
		...
		Success:    0,
		Failed:     0,
		Directions: make(map[DirectionSide]int),
...
type TestStats struct {
	...
	Success    int
	Failed     int
	Directions map[DirectionSide]int
...
func computeDirection(symbol string, orderflowData interface{}, mtfData interface{}) *DirectionArbitration {
	// 构建输入数据
...
func buildDirectionInput(symbol string, orderflowData interface{}, mtfData interface{}) *RootSymbolInput {
	// 解析订单流数据
...
func parseOrderflowData(data interface{}) *Orderflow {
	// 将 interface{} 转换为 map
...
// parseQuality 解析数据质量
func parseQuality(data map[string]interface{}) Quality {
	qualityData, ok := data["数据质量"].(map[string]interface{})
	if !ok {
		return Quality{Status: "异常", OverallScore: 0}
	}
...
// parseMacro 解析宏观资金趋势
func parseMacro(data map[string]interface{}) Macro {
...
func parseMicro5m(data map[string]interface{}) Micro5m {
...
func parseOrderbook(data map[string]interface{}) Orderbook {
...
// parseWall 解析墙体信息
func parseWall(data map[string]interface{}, key string) *Wall {
...
func parseMTFData(data interface{}) MTFAnalysis {
	rawData, ok := data.(map[string]interface{})
	if !ok {
		return nil
	}
...
// parseVPVR 解析VPVR数据
func parseVPVR(data map[string]interface{}) VPVR {
