...
func TestLong_FullMilestoneProgression(t *testing.T) {
	...
	steps := []step{
		...
		{"ROI=20%（锁14%）", 102000, 0.20, 0.14, true},
		{"ROI=25%（锁18%）", 102500, 0.25, 0.18, true},
		{"ROI=30%（锁22%）", 103000, 0.30, 0.22, true},
	}
...
func TestShort_FullMilestoneProgression(t *testing.T) {
	...
	steps := []step{
		...
		{"ROI=30%（锁22%）", 97000, 0.30, 0.22, true},
		{"ROI=50%（锁38%）", 95000, 0.50, 0.38, true},
	}
...
func TestRealScenario_DOGEUSDT_FullHistory(t *testing.T) {
	...
	priceSteps := []struct {
		...
		{0.091800 * 1.002, false, 0},     // 2% ROI → 未触发
		{0.091800 * 1.0052, true, 0.028}, // ~5.2% ROI → 命中5%里程碑，锁3%
		{0.091800 * 1.0082, true, 0.048}, // ~8.2% ROI → 命中8%里程碑，锁5%
		{0.091800 * 1.0122, true, 0.078}, // ~12.2% ROI → 命中12%里程碑，锁8%
		{0.091800 * 1.0202, true, 0.138}, // ~20.2% ROI → 命中20%里程碑，锁14%
		{0.093900, false, 0},               // 22.88% ROI → 仍在20%里程碑，止损已更新至该里程碑上限，无需再更新
		{0.091800 * 1.0252, true, 0.178}, // ~25.2% ROI → 命中25%里程碑，锁18%
	}
