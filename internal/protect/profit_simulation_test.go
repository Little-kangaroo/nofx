...
func TestLONGProfitSimulation(t *testing.T) {
	...
	priceSteps := []struct {
		...
		{90, 100300.0, 0.03, false, "价格上涨0.3%，ROI 3%，未触发"},
		{120, 100400.0, 0.04, false, "价格上涨0.4%，ROI 4%，未触发"},
		{150, 100500.0, 0.05, true, "价格上涨0.5%，ROI 5%，首次触发锁盈！"},
	...
	// 验证最终状态
	if updateCount != 6 {
		t.Errorf("期望6次止损更新，实际%d次", updateCount)
	}
...
func TestSHORTProfitSimulation(t *testing.T) {
	...
	priceSteps := []struct {
		...
		{90, 99700.0, 0.03, false, "价格下跌0.3%，ROI 3%，未触发"},
		{120, 99600.0, 0.04, false, "价格下跌0.4%，ROI 4%，未触发"},
		{150, 99500.0, 0.05, true, "价格下跌0.5%，ROI 5%，首次触发锁盈！"},
